...
import { UilFileCheck, UilTrash, UilFileImport, UilShareAlt } from '@iconscout/react-unicons';
...
const RenderItem = React.memo(({ item }: { item: IArrayAction }) => {
  ...
  return (
    <Tooltip placement="left" title={title}>
      <span className={`item__icon ${item.disable == true ? 'no-click' : ''}`} onClick={onClick}>
        <RenderIcon {...item} />
      </span>
    </Tooltip>
  );
